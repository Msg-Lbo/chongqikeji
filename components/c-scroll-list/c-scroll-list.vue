	...
	<view class="scroll-list-wrap" :style="[scrollListWrapStyle]">
		<scroll-view class="scroll-view" :class="[elClass]" :style="[listWrapStyle]" scroll-y scroll-anchoring
			...
			<view class="scroll-content" :style="[scrollContentStyle]">
				...
				<view class="pull-up-wrap flex flex-center flex-col" v-if="showPullUp">
			...
			getList(params, state, type) {
				...
				api(params)
					.then((res) => {
						...
						if (this.defaultOption.field) {
							if (state) {
								this.list = res.data[this.defaultOption.field].list;
							} else {
								this.list = params.pageNum > 1 ? this.list.concat(res.data[this.defaultOption.field].list) : res
									.data[this.defaultOption.field].list;
							}
							this.count = res.data[this.defaultOption.field].total;
							otherData = {...res.data}
							delete otherData[this.defaultOption.field]
							this[type + "Success"]({
								list: res.data[this.defaultOption.field].list,
								count: res.data[this.defaultOption.field].total,
							});
						} else {
							if (state) {
								this.list = list;
